    ...
    public static class ModelExtensions
    {
        ...
        public static Sdp ToSdp(this string str)
        {
            ...
            foreach (var token in tokens)
            {
                ...
                else if (token.StartsWith(Sdp.AttributeIndicator))
                {
                    ...
                    // Value attributes.
                        ...
                        Debug.LogWarning($"==== SDP unsupported media description attribute:{attr}");
                    ...
                    Debug.LogWarning($"==== SDP unsupported session field:{token}");
                //throw new NotSupportedException($"Unknown SDP field for session: {token}");

                idx++;
            }

            //Debug.Log("Parsing media section");
            ...
            foreach (var token in tokens)
            {
                ...
                else if (token.StartsWith(Sdp.AttributeIndicator))
                {
                    ...
                    Debug.LogWarning($"==== SDP unsupported media description field:{token}");
        ...
        public static Origin ToOrigin(this string str)
        {
            ...
            return new Origin
            {
                ...
                NetType = tokens[3].EnumFromStringValue<NetType>(),
                AddrType = tokens[4].EnumFromStringValue<AddrType>(),
        ...
        public static string ToText(this Origin origin) =>
            $"{Sdp.OriginIndicator}{origin.UserName} {origin.SessionId} {origin.SessionVersion} " +
                $"{origin.NetType.GetStringValue()} {origin.AddrType.GetStringValue()} {origin.UnicastAddress}" +
                $"{Sdp.CRLF}";
        ...
        public static ConnectionData ToConnectionData(this string str)
        {
            ...
            {
                NetType = tokens[0].EnumFromStringValue<NetType>(),
                AddrType = tokens[1].EnumFromStringValue<AddrType>(),
                ConnectionAddress = tokens[2]
            };
        }

        public static string ToText(this ConnectionData connectionData) =>
            $"{Sdp.ConnectionDataIndicator}{connectionData.NetType.GetStringValue()} " +
                $"{connectionData.AddrType.GetStringValue()} {connectionData.ConnectionAddress}" +
                $"{Sdp.CRLF}";
        ...
        public static Bandwidth ToBandwidth(this string str)
        {
            ...
            {
                Type = tokens[0].EnumFromStringValue<BandwidthType>(),
        ...
        public static EncryptionKey ToEncryptionKey(this string str)
        {
            ...
            {
                Method = tokens[0].EnumFromStringValue<EncryptionKeyMethod>(),
                Value = tokens[1]
            };
        }

        public static string ToText(this EncryptionKey encryptionKey) =>
            $"{Sdp.EncryptionKeyIndicator}{encryptionKey.Method.GetStringValue()}:{encryptionKey.Value}" +
                $"{Sdp.CRLF}";
        ...
        public static string ToText(this MediaDescription mediaDescription)
        {
            ...
            return $"{Sdp.MediaDescriptionIndicator}" +
                    ...
                    $"{(mediaDescription.Fmts is not null ? string.Join(" ", mediaDescription.Fmts) : string.Empty)}" +
                    $"{Sdp.CRLF}";
        }
        ...
        public static Fingerprint ToFingerprint(this string str)
        {
            ...
            {
                HashFunction = tokens[0].EnumFromStringValue<HashFunction>(),
        ...
        public static Rtcp ToRtcp(this string str)
        {
            ...
            return new Rtcp
            {
                ...
                NetType = tokens.Length > 3 ? tokens[1].EnumFromStringValue<NetType>() : null,
                AddrType = tokens.Length > 3 ? tokens[2].EnumFromStringValue<AddrType>() : null,
        ...
        public static string ToText(this Rtcp rtcp) =>
            $"{Sdp.AttributeIndicator}{Rtcp.Label}" +
                $"{rtcp.Port}{(rtcp.NetType.HasValue ? $" {rtcp.NetType.GetStringValue()}" : string.Empty)}" +
                $"{(rtcp.AddrType.HasValue ? $" {rtcp.AddrType.GetStringValue()}" : string.Empty)}" +
        ...
        public static Candidate ToCandidate(this string str)
        {
            ...
            var candidate = new Candidate
            {
                ...
                Transport = tokens[2].EnumFromStringValue<CandidateTransport>(),
                Priority = int.Parse(tokens[3]),
                ConnectionAddress = tokens[4],
                Port = int.Parse(tokens[5]),
                Type = tokens[7].EnumFromStringValue<CandidateType>(),
            };
            var idx = 8;
        ...
        public static Rid ToRid(this string str)
        {
            ...
            {
                Id = tokens[0],
                Direction = tokens[1].EnumFromStringValue<RidDirection>(),
            };
        ...
        public static string ToText(this Rid rid) =>
            $"{Sdp.AttributeIndicator}{Rid.Label}" +
                $"{rid.Id} {rid.Direction.GetStringValue()} " +
        ...
        public static string ToText(this Extmap extmap) =>
            $"{Sdp.AttributeIndicator}{Extmap.Label}" +
                 $"{extmap.Value}{(extmap.Direction.HasValue ? extmap.Direction.GetStringValue() : string.Empty)} " +
